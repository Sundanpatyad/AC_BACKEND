...
exports.verifyMockTestPayment = async (req, res) => {
  ...
  try {
    ...
    if (existingVerification) {
      console.log(existingVerification, "Payment already verified");
      return res
        .status(201)
    ...
    while (retryAttempts > 0 && !updateSuccess) {
      try {
        console.log("Start Updating Mocktest Series");
        ...
        if (updateResult.modifiedCount === mockTestIds.length) {
          console.log(" Updated Mocktest Series");
    ...
    console.log("Payment verified and access granted successfully.")
    return res.status(200).json({
      success: true,
        ...
        try {
          ...
          switch (req.body.event) {
            ...
            case "payment.captured": {
              ...
              if (!order) {
                console.log(`Order not found for order_id: ${event.order_id}`);
                throw new Error(
                  `Order not found for order_id: ${event.order_id}`
                );
              }
              ...
              if (user) {
                console.log(
                  `User ${order.userId} already has some of these mock tests`
                );
                await session.abortTransaction();
                session.endSession();
                return {
              ...
              if (existingEnrollments) {
                console.log(
                  `User ${order.userId} already enrolled in some mock test series`
                );
                await session.abortTransaction();
                session.endSession();
                return {
              ...
              for (const mockTestId of order.mockTestIds) {
                await MockTestSeries.updateOne(
                  ...
                  {
                    $addToSet: {
                      studentsEnrolled: new mongoose.Types.ObjectId(
                        order.userId
                      ),
                    },
                  },
              ...
              console.log(
                `Order processed successfully for user ${order.userId}`
              );
              break;
            }
          ...
          if (
            ...
            console.log(
              `Transaction attempt ${retryCount + 1} failed, retrying...`
            );
            retryCount++;
